...
Update column names in your data to Darwin Core terms. This also runs basic checks on your data to make sure each column is in the correct format. Fix columns that flag errors in your pipe directly within `use_` functions, which support data wrangling operations & `dplyr::mutate()` functionality.
...
Or, if your data is nearly ready and you want to run checks over all columns that match Darwin Core terms, run `check_occurrences()`. `check_occurrences()` only checks columns with column names that match valid Darwin Core terms.
